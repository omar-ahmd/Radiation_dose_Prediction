        ...
        if clas is None or weights is None:
            ...
            for i in range(2):
                ...
                if possible_dose_mask.sum() > 0:
                    ...
                    L1_loss += weights[i] * self.L1_loss_func(
...
class Loss_SM_ROI(nn.Module):
    ...
    def __init__(self, sm_weight=1.0):
        super().__init__()
    ...
    def forward(self, pred, gt, clas=None, weights=None):
        ...
        if clas is None or weights is None:
            ...
            if possible_dose_mask.sum() > 0 and structure_masks.sum() > 0:
                ...
                pdmloss = self.sl1(
                    pred_dose[possible_dose_mask > 0], gt_dose[possible_dose_mask > 0]
                )  # (bs)
                ...
                dvhloss = self.sl2(
                    sminput[structure_masks > 0], smtarget[structure_masks > 0]
                )  # (bs)
            ...
            for i in range(2):
                ...
                if possible_dose_mask.sum() > 0:
                    ...
                    L1_loss += weights[i] * (
                        self.L1_loss_func(pred_dose_c, gt_dose_c)
                        + self.sm_weight * dvhloss
                    )
...
class PTV_estimator_loss(nn.Module):
    ...
    def forward(self, pred, gt, clas=None, weights=None):
        ...
        if possible_dose_mask.sum() > 0:
            ...
            pdmloss = self.loss(
                pred[0] * possible_dose_mask, gt[0] * possible_dose_mask
            )  # (bs)
