...
def random_rotate_around_z_axis(
    list_images, list_angle, list_interp, list_boder_value, p=0.5
):
    if random.random() <= p:
        ...
        for image_i in range(len(list_images)):
            for chan_i in range(list_images[image_i].shape[0]):
                ...
                list_images[image_i][chan_i, :, :] = cv2.warpAffine(
...
def random_translate(list_images, roi_mask, p, max_shift, list_pad_value):
    if random.random() <= p:
        ...
        list_images = random_pad_to_size_2d(
            list_images, target_size=[ori_h, ori_w], list_pad_value=list_pad_value
        )
