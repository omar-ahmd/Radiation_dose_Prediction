...
def read_data(patient_dir):
    dict_images = {}
    list_structures = ["ct", "possible_dose_mask", "structure_masks", "dose"]
...
def pre_processing(dict_images, with_distance=True, PTV_estimate=False, without_PTVs=False):
    ...
    if not PTV_estimate and not without_PTVs:
        PTVs = 70.0 / 70.0 * PTVs[2] + 63.0 / 70.0 * PTVs[1] + 56.0 / 70.0 * PTVs[0]
        if with_distance:
            ...
            list_images = [
                np.concatenate(
                    (PTVs[None], OAR_all, CT[None], distance_image), axis=0
                ),  # Input
                dose[None],  # Label
            ...
            list_images = [
                np.concatenate((PTVs[None], OAR_all, CT[None]), axis=0),  # Input
                dose[None],  # Label
                possible_dose_mask[None],
                structure_masks,
            ]

    elif not without_PTVs:
        PTVs = np.concatenate(
            (
                PTVs,
                (1 * (PTVs.sum(axis=0) == 0) * possible_dose_mask)[None],
                (1 * (possible_dose_mask == 0))[None],
            ),
            axis=0,
        )

        list_images = [
            np.concatenate((CT[None], OAR_all), axis=0),  # Input
            PTVs,  # Label
            possible_dose_mask[None],
            dose[None],
        ]
...
class MyDataset(data.Dataset):
    def __init__(
        ...
        self.without_PTVs = without_PTVs
        self.transform = {
        ...
        if resplit:
            ...
            self.list_case_id = {
                ...
                "val": train_val[split:],
                "test": [a[0] for a in os.walk(dir + "/test/") if "sample" in a[0]],
            }[phase]

        else:
            self.list_case_id = {
                "train": [a[0] for a in os.walk(dir + "/train/") if "sample" in a[0]],
                "val": [
                    a[0] for a in os.walk(dir + "/validation/") if "sample" in a[0]
                ],
                "test": [a[0] for a in os.walk(dir + "/test/") if "sample" in a[0]],
            }[phase]
    ...
    def __getitem__(self, index_):
        ...
        list_images = pre_processing(
            dict_images,
            with_distance=self.with_distance,
            PTV_estimate=self.PTV_estimate,
            without_PTVs=self.without_PTVs
        )

        if (
            self.phase == "train"
            and not self.PTV_estimate
            and (
            ...
            if (
                list_images[1][0:3, :, :].sum() == 0
                or list_images[2][0, :, :].sum() == 0
            ):
                return self.__getitem__(np.random.randint(self.__len__()))
...
def get_loader(
    ...
    train_dataset = MyDataset(
        dir,
        num_samples_per_epoch=train_num_samples_per_epoch,
        phase="train",
        resplit=resplit,
        seed=seed,
        train_size=train_size,
        ...
        with_distance=with_distance,
        PTV_estimate=PTV_estimate,
        without_PTVs=without_PTVs
    )

    val_dataset = MyDataset(
        dir,
        num_samples_per_epoch=val_num_samples_per_epoch,
        phase="val",
        resplit=resplit,
        seed=seed,
        train_size=train_size,
